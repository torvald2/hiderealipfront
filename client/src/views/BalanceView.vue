    ...
    .row.gx-4.gx-lg-5.row-cols-2.row-cols-md-3.row-cols-xl-4.justify-content-center
      ...
      .col.mb-5
        .card.h-100
          ...
          .card-footer.p-4.pt-0.border-top-0.bg-transparent
            .text-center
              a.btn.btn-outline-dark.mt-auto(href='#') Open this Setup
    b-modal(id="modal-1" title="Bind IP")
      b-form
        b-form-group(
            id="input-group-1"
            label="Domain name"
            description="Will be secured by blockchain database"
        )
          b-form-input(id="input-1"  type="text" v-model="domain_name" required)
        b-form-group(
            id="input-group-2"
            label="Your Real IP"
        )
  ...
  if (!response.ok) {
    throw new Error('Error');
      ...
      try {
        ...
        const contractAddress = "0xAF524e354560BbBe59D955fC8223981AfB3C3F81";
        ...
        const tokenCount = await nftContract.methods.balanceOf(userAddress).call();
